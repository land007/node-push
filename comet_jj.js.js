...
//	var child_process = require('child_process');
	...
	console.log('{"start port":' + port + ', "start time":' + new Date() + '}');
	var tickTime = 1000;
	var waitTime = 100;
	var resetTime = 10000;
	global.id = 1;
...
//		       updatedtime: 1393557170000 } ] }
		...
		msgsdb.find({ valid: true }, function (err, docs) {
			if (!err) {
				console.log('{"msgsdb.docs.length":' + docs.length + '}');
		...
		usersdb.find({ valid: true }, function (err, docs) {
			if (!err) {
				console.log('{"usersdb.docs.length":' + docs.length + '}');
				for (var i = 0; i < docs.length; i++) {
					...
					if (global.uuidUsers[docs[i]['uuid']]['userid'] !== undefined && global.uuidUsers[docs[i]['uuid']]['app'] === undefined) {
						...
						usersdb.update({ _id: global.uuidUsers[docs[i]['uuid']]['_id'] }, global.uuidUsers[docs[i]['uuid']], {}, function (err, numReplaced) {
							if (err) {
				        		console.error('{"usersdb.update err":"' + err + '"}');
				    		}
						});
					}
				}
		...
		setInterval(function() {
			...
			console.log('{"clean session time":' + now + '}');
			var i = 0;
			...
			console.log('{"clean session count":' + i + '}');
					...
					if (now > timeout) {
						if (message['_id'] !== undefined) {
							...
							msgsdb.update({ _id: message['_id'] }, message, {}, function (err, numReplaced) {
								if (err) {
									console.error('{"msgsdb.update err":"' + err + '"}');
								}
							});
						}
						messages.splice(i, 1);
						i--;
						console.log('{"splice":"' + JSON.stringify(message) + '", "messages.length":"' + messages.length + '"}');
						continue;
					} else {
...
//						console.log('{"GET Request file":"' + pathname + '"}');
...
//						console.log('{"GET Request path":"' + resid + '"}');
						var arg = url.parse(request.url).query; //arg => name=a&id=5  
//				        console.log('{"GET Request arg":"' + arg + '"}');
							...
							if (token !== undefined && clid !== undefined && sid !== undefined && sessionid !== undefined) {
			        			...
			        			setTimeout(function() {
			        				if (global.sessions[sessionid] !== undefined) {
										var session = global.sessions[sessionid];
//										console.log('{"global.uuidInterval[session.uuid]":"' + global.uuidInterval[session.uuid] + '"}');
										if (global.uuidInterval[session.uuid] !== undefined) {
//											console.log('{"connect":"reset","sessionid":"' + sessionid + '","ip":"' + global.uuidInterval[session.uuid]['ip'] + '"}');
...
//													console.log('{"userid":"' + session['userid'] + '", "usertime":"' + session['usertime'] + '"}');
												}
												session['fire'] = global.uuidUsers[session.uuid]['fire'];
											}
					        			}, waitTime);
										var ip = getClientIp(request) + ':' + getClientPort(request);
//										console.log('{"connect":"open","sessionid":"' + sessionid + '","ip":"' + ip + '"}');
										session['ip'] = ip;
										setStaticContentType(headers);
										response.writeHead(200, headers);
										var myoutput = function(first) {
											if (session.assert !== undefined) {
//												console.log('{"connect":"end","sessionid":"' + sessionid + '","ip":"' + global.uuidInterval[session.uuid]['ip'] + '"}');
												clearInterval(global.uuidInterval[session.uuid]['interval']);
												global.uuidInterval[session.uuid]['response'].end();
												delete global.sessions[global.uuidInterval[session.uuid]['sessionid']];
												delete global.uuidInterval[session.uuid];
...
//																}
																...
																if (now > timeout) {
																	...
																	if (session['fire'] === undefined || (session['fire'] !== undefined && session['fire'])) {
																		...
																		console.log('{"global.uuidMessages":"' + docs[i]['msg'] + '"}');
																	}
																}
																(function(id, doc, del) {
																	userMsgsdb.update({ _id: id }, doc, {}, function (err, numReplaced) {
																		if (err) {
																			console.error('{"userMsgsdb.update":"' + err + '"}');
																		} else {
																			if (del) {
																				console.log('{"userMsgsdb.id":"' + id + '"}');
																				userMsgsdb.remove({ _id: id }, function (err, numRemoved) {
																					if (err) {
																						console.error('{"userMsgsdb.remove":"' + err + '"}');
																					} else {
																						console.log('{"userMsgsdb.remove":"' + id + '"}');
																					}
																				});
																			}
																		}
...
//											var timeout = 
											setTimeout(function() {
												if (global.uuidMessages[session.uuid] !== undefined) {//  && global.uuidTimeout[session.uuid] === undefined
													...
													str = (str.length-1) + str;
//													console.log('{"session.uuid":"' + session.uuid + '", "msg":"' + str + '"}');
													response.write(str);
													session['n']++;
													session['noop'] = now;
...
//											console.log('{"connect":"closed","sessionid":"' + sessionid + '","ip":"' + ip + '"}');
...
//										console.error('{"GET Request unknown session":"' + sessionid + '", "global.id":' + global.id+'}');
...
//								response.end('[]');
								...
								if (global.uuidUsers[uuid] === undefined) {
									...
									usersdb.insert(global.uuidUsers[uuid], function (err, newMsg) {
							        	if (err) {
							        		console.error('{"usersdb.insert err":"' + err + '"}');
									...
									if (global.uuidUsers[uuid]['_id'] !== undefined) {
										console.log('{"signUser":"ok", "from":"' + global.uuidUsers[uuid]['userid'] + '_' + global.uuidUsers[uuid]['time'] + '", "to":"' + userid + '_' + now + '"}');
										global.uuidUsers[uuid]['uuid'] = uuid;
										global.uuidUsers[uuid]['userid'] = userid;
										global.uuidUsers[uuid]['time'] = now;
										global.uuidUsers[uuid]['fire'] = true;
										...
										console.error('{"signUser err":"' + err + '"}');
									}
								}
								for (var sessionid in global.sessions) {
									var session = global.sessions[sessionid];
								...
								if (uuid !== undefined) {
									...
									if (global.uuidUsers[uuid]['_id'] !== undefined) {
										...
										usersdb.insert(global.uuidUsers[uuid], function (err, newMsg) {
											if (err) {
												console.error('{"usersdb.update insert":"' + err + '"}');
											} else {
...
//								response.end('{"token":"0e1137508a574ebeb06a137d48f4f23b"}\n');
			...
			'POST' : function(resid, data, request, response) {
			    ...
			    request.addListener("end", function () {
					...
					headers['Server'] = 'JSE';
					headers['Version'] = 'HTTP/1.1';
//					console.log('{"POST Request path":"' + resid + '"}');
//			        console.log('{"POST Request arg":"' + arg + '"}');
			        var args = null;
			        if (arg != null) {
...
//				        			console.error('{"POST Request unknown session":"' + sessionid + '", "global.id":' + global.id + '}');
			        		...
			        		console.error('{"POST Request unknown token":"' + token + '", "global.id":' + global.id + '}');
			        		response.writeHead(400);
							response.end('unknown token\n');
			        	}
...
//		test('is.js');
	...
	global.ep0 = EventProxy.create('WorkerId', function(worker) {
		...
		console.log('{"global.isWorker":' + global.isWorker + ', "global.id":' + global.id + '}');
		var dbpath = 'data/';
		var db = {};
		...
		db['userMsgs'].loadDatabase(function (err) {
			if (!err) {
				...
				console.error('{"Load UserMsgs error":"' + err + '"}');
			}
		});
...
//	});
	...
	if (global.isWorker) {
		test('bs_start.js', function(data) {
			console.log('{"test":"success", "global.id":' + global.id+'}');
			test = data;
			test(jse + '/iw.js');
//			test('iw.js');
//			test('ih.js');
